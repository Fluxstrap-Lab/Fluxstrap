    ...
    static class Utilities
    {
        ...
        public static VersionComparison CompareVersions(string versionStr1, string versionStr2)
        {
            ...
            catch (Exception)
            {
                ...
                // https://github.com/bloxstraplabs/Fluxstrap/issues/3193
        ...
        {
            using EventWaitHandle handle = new EventWaitHandle(false, EventResetMode.AutoReset, "Fluxstrap-BackgroundUpdaterKillEvent");
            handle.Set();
        }
        ...
        public static void OpenFolderInExplorer(string filePath)
        {
            try
            {
                string folder = File.GetAttributes(filePath).HasFlag(FileAttributes.Directory)
                    ? filePath
            ...
            {
                App.Logger.WriteException("Utilities::OpenFolderInExplorer", ex);
            }
        }

        public static void SetStartupLaunch(bool enable)
        {
            const string LOG_IDENT = "Utilities::SetStartupLaunch";
            ...
            try
            {
                ...
                if (enable)
                    key.SetValue(appName, $"\"{Paths.Application}\" -autostart");
                else if (key.GetValue(appName) is not null)
                    key.DeleteValue(appName);

                App.Logger.WriteLine(LOG_IDENT, $"Startup launch {(enable ? "enabled" : "disabled")}");
            }
            catch (Exception ex)
            {
                App.Logger.WriteException(LOG_IDENT, ex);
