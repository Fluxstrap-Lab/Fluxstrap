namespace Fluxstrap.Integrations
{
    ...
    {
        private const string GameMessageEntry                = "[FLog::Output] [FluxstrapRPC]";
        ...
        private const string GameMessageEntryPattern         = @"\[FluxstrapRPC\] (.*)";
        ...
        public event EventHandler<string>? OnServerBlocked;
        ...
        private void ReadLogEntry(string entry)
        {
            ...
            else if (!InGame && Data.PlaceId != 0)
            {
                ...
                else if (logMessage.StartsWith(GameJoinedEntry))
                {
                    ...
                    if (App.Settings.Prop.EnableServerBlacklist && Data.MachineAddressValid)
                        _ = CheckServerBlocked();

                    if (App.Settings.Prop.GameProfiles.Count > 0)
                        ApplyGameProfile(Data.UniverseId);
                }
            }
            else if (InGame && Data.PlaceId != 0)
            {
        ...
        private async Task CheckServerBlocked()
        {
            const string LOG_IDENT = "ActivityWatcher::CheckServerBlocked";

            try
            {
                string? countryCode = await Data.QueryServerCountryCode();
                if (string.IsNullOrEmpty(countryCode))
                    return;

                if (App.Settings.Prop.BlockedServerRegions.Contains(countryCode, StringComparer.OrdinalIgnoreCase))
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Server in blocked region: {countryCode}");
                    OnServerBlocked?.Invoke(this, countryCode);
                }
            }
            catch (Exception ex)
            {
        ...
        private static void ApplyGameProfile(long universeId)
        {
            const string LOG_IDENT = "ActivityWatcher::ApplyGameProfile";

            try
            {
                var profile = App.Settings.Prop.GameProfiles.FirstOrDefault(x => x.UniverseId == universeId);
                if (profile is null)
                    return;

                App.Logger.WriteLine(LOG_IDENT, $"Applying profile for {profile.GameName} (universeId={universeId})");

                if (profile.UseCustomFpsCap)
                {
                    App.Settings.Prop.UnlockFPS = true;
                    App.Settings.Prop.FPSCap = profile.FpsCap;
                    App.Settings.Save();
                }

                if (profile.UseCustomGraphicsQuality)
                {
                    App.Settings.Prop.ForceGraphicsQuality = true;
                    App.Settings.Prop.GraphicsQualityLevel = profile.GraphicsQualityLevel;
                    App.Settings.Save();
                }
            }
            catch (Exception ex)
