...
using Fluxstrap.UI.Elements.Bootstrapper.Base;
...
namespace Fluxstrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"Fluxstrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "Fluxstrap-Bootstrapper";
        ...
        public async Task Run()
        {
            ...
            if (App.Settings.Prop.UseGoodbyeDPI)
            {
                App.Logger.WriteLine(LOG_IDENT, "GoodbyeDPI is enabled, starting...");
                SetStatus(Strings.Bootstrapper_Status_StartingGoodbyeDPI);
        ...
        private void StartRoblox()
        {
            ...
            if (!string.IsNullOrEmpty(App.Settings.Prop.CustomLaunchArgs))
                startInfo.Arguments = $"{App.Settings.Prop.CustomLaunchArgs} {startInfo.Arguments}";
            ...
            if (_appPid != 0)
            {
                ...
                if (App.Settings.Prop.OptimizeRoblox)
                {
                    const string PERF_IDENT = "Bootstrapper::ApplyOptimizations";
                    App.Logger.WriteLine(PERF_IDENT, "Applying runtime performance optimizations...");

                    try
                    {
                        // Boost Roblox process to High priority for smoother gameplay
                        using var robloxProcess = Process.GetProcessById(_appPid);
                        robloxProcess.PriorityClass = ProcessPriorityClass.High;
                        App.Logger.WriteLine(PERF_IDENT, $"Set Roblox priority to High");
                    }
                    catch (Exception ex)
                    {
                        App.Logger.WriteLine(PERF_IDENT, $"Failed to boost priority: {ex.Message}");
                    }

                    // Set 1ms timer resolution for smoother frame pacing
                    try
                    {
                        Utility.AggressivePerformanceManager.SetTimerResolution();
                        App.Logger.WriteLine(PERF_IDENT, "Set high-resolution timer (1ms)");
                    }
                    catch (Exception ex)
                    {
                        App.Logger.WriteLine(PERF_IDENT, $"Failed to set timer resolution: {ex.Message}");
                    }

                    // Set process I/O priority to High for faster asset loading
                    try
                    {
                        using var robloxProcess = Process.GetProcessById(_appPid);
                        robloxProcess.PriorityBoostEnabled = true;
                        App.Logger.WriteLine(PERF_IDENT, "Enabled priority boost for Roblox");
                    }
                    catch (Exception ex)
                    {
                        App.Logger.WriteLine(PERF_IDENT, $"Failed to set priority boost: {ex.Message}");
                    }
                }

                int maxCores = App.Settings.Prop.MaxCpuCores;
                if (maxCores > 0)
                    CpuCoreLimiter.SetProcessCpuCoreLimit(_appPid, maxCores);

                if (App.Settings.Prop.MemoryCleaner)
                    RobloxMemoryCleaner.OptimizeProcessMemory(_appPid);
            }

            // Call Lua script execution
            ...
            if (App.Settings.Prop.EnableActivityTracking || App.LaunchSettings.TestModeFlag.Active || App.Settings.Prop.UseGoodbyeDPI || autoclosePids.Any())
            {
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one Fluxstrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingFluxstrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "Fluxstrap.exe");
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.GetType() == typeof(ChecksumFailedException))
                    {
                        ...
                        Frontend.ShowConnectivityDialog(
                            ...
                            String.Format(Strings.Dialog_Connectivity_UnableToDownloadReason, "[https://bloxstraplabs.com/wiki/help/Fluxstrap-cannot-download-roblox/](https://bloxstraplabs.com/wiki/help/Fluxstrap-cannot-download-roblox/)"),
        ...
        {
            if (!App.Settings.Prop.UnlockFPS)
                return;

            const string LOG_IDENT = "Bootstrapper::ApplyFPSCap";
            App.Logger.WriteLine(LOG_IDENT, "Applying FPS cap...");
            ...
            try
            {
                ...
                if (!editor.Loaded)
                {
                    App.Logger.WriteLine(LOG_IDENT, "GBS file not loaded, skipping");
                    return;
                }

                editor.SetPreset("Rendering.FramerateCap", App.Settings.Prop.FPSCap);
                editor.Save();

                App.Logger.WriteLine(LOG_IDENT, $"FPS cap set to {App.Settings.Prop.FPSCap}");
            }
            catch (Exception ex)
            {
                App.Logger.WriteException(LOG_IDENT, ex);
        ...
        {
            if (!App.Settings.Prop.ForceGraphicsQuality)
                return;

            const string LOG_IDENT = "Bootstrapper::ApplyGraphicsQuality";
            App.Logger.WriteLine(LOG_IDENT, "Applying graphics quality override...");
            ...
            try
            {
                ...
                editor.SetPreset("Rendering.SavedQualityLevel", App.Settings.Prop.GraphicsQualityLevel);
                editor.Save();

                App.Logger.WriteLine(LOG_IDENT, $"Graphics quality set to {App.Settings.Prop.GraphicsQualityLevel}");
            }
            catch (Exception ex)
            {
                App.Logger.WriteException(LOG_IDENT, ex);
